  ...
  passport.authenticate("jwt", { session: false }, (err, user, info) => {
    ...
    if (info !== undefined) {
      ...
      for (i = 0; i < req.body.orders.length; i++) {
        ...
        const unitOrder = new OrderModels.unitorder({
          territoryName: req.body.orders[i][0],
          order: req.body.orders[i][1],
        });
      ...
      Game.findOne({ gameid: req.params.id }, (err, game) => {
        ...
        OrderModels.orders.findByIdAndUpdate(
          orderid,
          { [req.body.country]: nationsOrders },
...
router.post("/resolveorderstest/:id", (req, res, next) => {
  ...
  Game.findOne({ gameid: req.params.id }, (err, doc) => {
    ...
    const game = doc.toObject();
    const gameArray = Object.entries(game).filter(
      (item) => item[1].hasOwnProperty("occupied") && item[1]["occupied"]
    );
    const allTerritories = Object.entries(game).filter((item) =>
      item[1].hasOwnProperty("occupied")
    );
    let supCenterArray = [];
    for (var i = 0; i < allTerritories.length; i++) {
      if (
        BoardInfo[allTerritories[i][0]]["supply"] &&
        allTerritories[i][1]["owner"] !== "None"
      ) {
        supCenterArray.push([
          BoardInfo[allTerritories[i][0]]["abbrev"],
          allTerritories[i][1]["owner"],
        ]);
      }
    }
    ...
    for (var i = 0; i < dislodged.length; i++) {
      dislodgedArray.push([
        dislodged[i]["territoryName"],
        Object.fromEntries(
          new Map([
            ["Type", dislodged[i]["Type"]],
            ["Nation", dislodged[i]["Nation"]],
          ])
        ),
      ]);
    }
    ...
    for (var i = 0; i < bounces.length; i++) {
      ...
      bouncesArray.push([
        bounces[i]["territory"],
        Object.fromEntries(new Map(subBounceArray)),
      ]);
    }
    ...
    for (var i = 0; i < gameArray.length; i++) {
      ...
      var obj = Object.fromEntries(
        new Map([
          ["Type", gameArray[i][1]["occupier"]["force"]],
          ["Nation", gameArray[i][1]["occupier"]["country"]],
        ])
      );
      gameArray[i][1] = obj;
    }
    ...
    OrderModels.orders.findById(doc.get("currentMoveId"), (err, orders) => {
      const ordersArray = Object.entries(orders.toObject()).filter(
        (item) => item[0] !== "_id" && item[0] !== "__v"
      );
      console.log(ordersArray[0][1]);
      for (var i = 0; i < ordersArray.length; i++) {
        var countryOrderArray = [];
        for (var j = 0; j < ordersArray[i][1].length; j++) {
          countryOrderArray.push([
            ordersArray[i][1][j]["territoryName"],
            ordersArray[i][1][j]["order"],
          ]);
        }
      ...
      const apiData = JSON.stringify(
        Object.fromEntries(
          new Map([
            ...
            ["Type", type],
            ["Units", unitsObj],
            ["SupplyCenters", supplyCenterObj],
            ["Dislodgeds", dislodgedsObj],
            ["Dislodgers", dislodgersObj],
            ["Bounces", bouncesObj],
            ["Orders", ordersObj],
            ["Resolutions", {}],
          ])
      ...
      axios(config)
        .then(function (response) {
          ...
          const territoryArray = Object.entries(game).filter((item) =>
            item[1].hasOwnProperty("occupied")
          );
          ...
          for (var i = 0; i < territoryArray.length; i++) {
            ...
            if (UnitsAbbrevsArray.includes(abbrev)) {
              ...
              if (
                (abbrev === "spa" || abbrev === "bul" || abbrev === "stp") &&
                coastalProvs[abbrev]
              ) {
                continue;
              }
          ...
          const new_orders = new OrderModels.orders({
            _id: new_orders_id,
            season: adjudication["Season"],
